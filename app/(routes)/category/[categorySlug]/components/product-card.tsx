...
const ProductCard = (props: ProductCardProps) => {
  ...
  return (
    <Link href={`/product/${product.slug}`} className="relative p-2 transition-all duration-100 rounded-lg hover:shadow-md">
       ...
       <p className="px-2 py-1 text-xs text-white bg-yellow-900  dark:bg-white dark:text-black w-fit">{product.origin}</p>
       </div>
        ...
        <CarouselContent>
          {product.images?.map((image)=>(
            <CarouselItem key={image.id} className="group">
               <img src={`${process.env.NEXT_PUBLIC_BACKEND_URL}${image.attributes.url}`} alt="Image" className="rounded=xl"/> {/* eslint-disable-line @next/next/no-img-element */}
